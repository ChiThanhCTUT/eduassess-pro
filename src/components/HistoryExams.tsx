            ...
            <div className="flex items-center gap-4">
              <div className="w-12 h-12 rounded-xl bg-gray-50 border border-gray-200 flex items-center justify-center text-[#0058be]">
                <span className="material-symbols-outlined text-2xl">
                  {item.iconName === 'functions'
                    ? 'functions'
                    : item.iconName === 'biotech'
                      ? 'biotech'
                      : item.iconName === 'psychology'
                        ? 'psychology'
                        : 'code'}
                </span>
              </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">
                    Báo cáo kiểm duyệt kết quả kiểm tra
                  </p>
                </div>
              </div>
              <button
            ...
            <div className="p-6 overflow-y-auto space-y-6 flex-1 text-left">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 p-5 bg-[#f8f9fa] border border-gray-200 rounded-2xl">
                ...
                <div>
                  ...
                  <span
                    className={`inline-block px-2.5 py-0.5 rounded text-[10px] font-bold uppercase tracking-wide mt-2 ${
                      selectedExam.result === 'Đạt'
                        ? 'bg-green-100 text-green-700'
                        : 'bg-red-100 text-red-700'
                    }`}
                  >
              ...
              <div>
                <h3 className="font-bold text-sm text-gray-500 uppercase tracking-wider mb-4">
                  Danh sách câu hỏi & Rà soát bài thi
                </h3>
                <div className="space-y-4">
                  {selectedExam.questionsDetail && selectedExam.questionsDetail.length > 0 ? (
                    selectedExam.questionsDetail.map(q => (
                      ...
                      >
                        <div className="flex justify-between items-start gap-4">
                          <span
                            className={`w-6 h-6 flex items-center justify-center rounded-full font-bold text-xs flex-shrink-0 ${
                              q.isCorrect
                          ...
                          <div className="flex-1">
                            <h4 className="text-sm font-semibold text-[#191c1d] leading-relaxed">
                              {q.questionText}
                            </h4>
                    ...
                    <p className="text-xs text-gray-400">
                      Không có thông tin chi tiết các câu trả lời.
                    </p>
                  )}
                </div>
              </div>
